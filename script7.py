#########################################################################
#This script can be used to estimate NGAM in 18h6h growth condition 	#
#based on the assumption that carbon conversion efficiency (CCE) of 	#
#night time metaoblism is 50%						#
#									#
#########################################################################
...
cobra_model.reactions.get_by_id("SUCROSE_v_dielTransfer").lower_bound=0
...
#Reset stoichiometry of diel_biomass so that day-night phloem export (per sec) is maintained at 3:1 
rxn = cobra_model_pH_18h6h.reactions.get_by_id("diel_biomass")
rxn.add_metabolites({cobra_model_pH_18h6h.metabolites.get_by_id("X_Phloem_contribution_t1"):((-3.0*18)/12) - (-3),cobra_model_pH_18h6h.metabolites.get_by_id("X_Phloem_contribution_t2"):((-1.0*6)/12 - (-1))})
