#########################################################################
#This script can be used to model different CAM substypes across a wide #
#range of VcVo ratios and compare their metabolic productivity	 	#
#									#
#########################################################################
...
################################### Vc/Vo CRUVE FOR DIFFERENT CAM SUBTYPES #############

#################### starch storing PEPCK CAM
...
writeAllFluxes(CAM_model_pH,solutiondict,"PEPCK_starch.csv")

################### sucrose storing PEPCK CAM
test_model=CAM_model_pH.copy()
test_model.reactions.get_by_id("MALIC_NADP_RXN_c1").lower_bound=0
test_model.reactions.get_by_id("MALIC_NADP_RXN_c1").upper_bound=0
test_model.reactions.get_by_id("MALIC_NADP_RXN_p1").lower_bound=0
...
test_model.reactions.get_by_id("SUCROSE_v_dielTransfer").lower_bound = -1000
test_model.reactions.get_by_id("SUCROSE_v_dielTransfer").upper_bound = 1000
test_model.reactions.get_by_id("FRUCTAN_v_dielTransfer").lower_bound=0
test_model.reactions.get_by_id("FRUCTAN_v_dielTransfer").upper_bound=0
solution=flux_analysis.parsimonious.optimize_minimal_flux(test_model)
...
for i in [1000,160,80,40,20,10,5,3,2]:
  ...
  temp.reactions.get_by_id("SUCROSE_v_dielTransfer").lower_bound = -1000
  temp.reactions.get_by_id("SUCROSE_v_dielTransfer").upper_bound = 1000
  temp.reactions.get_by_id("FRUCTAN_v_dielTransfer").lower_bound=0
  temp.reactions.get_by_id("FRUCTAN_v_dielTransfer").upper_bound=0
  temp.reactions.get_by_id("RXN_961_p1").add_metabolites({Rubisco_balance:-b})
  temp.reactions.get_by_id("RIBULOSE_BISPHOSPHATE_CARBOXYLASE_RXN_p1").add_metabolites({Rubisco_balance:1})
...
writeAllFluxes(CAM_model_pH,solutiondict,"PEPCK_sucrose.csv")


################### fructan-storing PEPCK CAM
test_model=CAM_model_pH.copy()
test_model.reactions.get_by_id("MALIC_NADP_RXN_c1").lower_bound=0
test_model.reactions.get_by_id("MALIC_NADP_RXN_c1").upper_bound=0
test_model.reactions.get_by_id("MALIC_NADP_RXN_p1").lower_bound=0
...
ansB1 = dict()
ansB1[10000]=test_model.solution.f
...
for i in [1000,160,80,40,20,10,5,3,2]:
  ...
  print "Storing results from "+str(b)
  ansB1[b]=test_model.solution.f
  solutiondict[b]=test_model.solution.x_dict
  #break



writeAllFluxes(CAM_model_pH,solutiondict,"PEPCK_fructan.csv")



##################### starch storing ME CAM
test_model=CAM_model_pH.copy()
test_model.reactions.get_by_id("PEPCARBOXYKIN_RXN_c1").lower_bound = 0
test_model.reactions.get_by_id("PEPCARBOXYKIN_RXN_c1").upper_bound = 0
test_model.reactions.get_by_id("STARCH_p_dielTransfer").lower_bound = -1000
test_model.reactions.get_by_id("STARCH_p_dielTransfer").upper_bound = 1000
test_model.reactions.get_by_id("SUCROSE_v_dielTransfer").lower_bound = 0
test_model.reactions.get_by_id("SUCROSE_v_dielTransfer").upper_bound = 0
...
for i in [1000,160,80,40,20,10,5,3,2]:
  ...
  temp.reactions.get_by_id("FRUCTAN_v_dielTransfer").lower_bound=0
  temp.reactions.get_by_id("FRUCTAN_v_dielTransfer").upper_bound=0
  temp.reactions.get_by_id("PEPCARBOXYKIN_RXN_c1").lower_bound = 0
  temp.reactions.get_by_id("PEPCARBOXYKIN_RXN_c1").upper_bound = 0
  temp.reactions.get_by_id("RXN_961_p1").add_metabolites({Rubisco_balance:-b})
  temp.reactions.get_by_id("RIBULOSE_BISPHOSPHATE_CARBOXYLASE_RXN_p1").add_metabolites({Rubisco_balance:1})
  solution=flux_analysis.parsimonious.optimize_minimal_flux(temp)
...
writeAllFluxes(CAM_model_pH,solutiondict,"ME_starch.csv")





###################### sucrose-storing ME CAM
test_model=CAM_model_pH.copy()
test_model.reactions.get_by_id("PEPCARBOXYKIN_RXN_c1").lower_bound = 0
test_model.reactions.get_by_id("PEPCARBOXYKIN_RXN_c1").upper_bound = 0
test_model.reactions.get_by_id("STARCH_p_dielTransfer").lower_bound = 0
...
writeAllFluxes(CAM_model_pH,solutiondict,"ME_sucrose.csv")



###################### fructan-storing ME CAM
test_model=CAM_model_pH.copy()
test_model.reactions.get_by_id("PEPCARBOXYKIN_RXN_c1").lower_bound = 0
test_model.reactions.get_by_id("PEPCARBOXYKIN_RXN_c1").upper_bound = 0
test_model.reactions.get_by_id("STARCH_p_dielTransfer").lower_bound = 0
...
ansD1 = dict()
ansD1[10000]=test_model.solution.f
...
writeAllFluxes(CAM_model_pH,solutiondict,"ME_fructan.csv")



######################################Plot metabolic productivities with y axis as % of C3#####################
...
for k in sorted(ansA.keys()):
  ...
  yvalues3.append(((ansB1.get(k))/temp_model3.solution.f)*100)
  yvalues4.append(((ansC.get(k))/temp_model3.solution.f)*100)
  yvalues5.append(((ansD.get(k))/temp_model3.solution.f)*100)
  yvalues6.append(((ansD1.get(k))/temp_model3.solution.f)*100)

curve = plt.subplot(111)
curve.plot(xvalues2,yvalues,marker="o",label="Starch storing PEPCK CAM",color="blue")
curve.plot(xvalues2,yvalues2,marker="o",label="Sucrose storing PEPCK CAM",color="red")
curve.plot(xvalues2,yvalues3,marker="o",label="Fructan storing PEPCK CAM",color="black")
curve.plot(xvalues2,yvalues4,marker="o",label="Starch storing ME CAM",color="brown")
curve.plot(xvalues2,yvalues5,marker="o",label="Sucrose storing ME CAM",color="orange")
curve.plot(xvalues2,yvalues6,marker="o",label="Fructan storing ME CAM",color="purple")
